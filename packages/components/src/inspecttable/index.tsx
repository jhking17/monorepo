...
export const InspectTableCompnent : React.FunctionComponent<FinaltableProps> = ( props : FinaltableProps )=>{
					...
					{props.data.map((row,idx) => (
						<TableBodyRow 
							...
							{Object.values(row).map((value,val_idx)=>{
								...
								} else if(Object.keys(props.data[0])[val_idx].indexOf("결과") != -1){
									return (
										<TableBodyCell 
											key={"tableCell" + val_idx} 
											align="center"
											style={{width : "80px", height : "48px", padding : "10px"}}
											padding="none"
											>
...
const TableBodyCell = styled(TableCell)`
	...
	width : fit-content;
	background-color : ${(props : styleProps) => props.$isFailed ? `#FEE4E5` : `transparent`};
	padding : 10px;
`;
...
const TableBodyCellResultDiv = styled.div`
	...
	align-items : center;
	background-color : ${(props : styleProps)=> props.$result ? `#43CB61` : `#DE5E61`};
	width : 100%;
	color : white;
	height : 100%;
`;