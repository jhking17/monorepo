...
export const EquipnoptableComponent : React.FunctionComponent<FinalequipnoptableProps> = ( props )=>{
                        ...
                        if(props.headObjList && props.headObjList.indexOf(raw) != -1){
                            return (
                                <TableHeadCell 
                                    ...
                                    $paddingNone
								    >
                                    <TableHeadObjGroupDiv>{raw}</TableHeadObjGroupDiv>
                                    <TableHeadObjGroupDiv>
                                    {Object.keys(val).map((option,idx)=>{
                                        return (
                                            <TableHeadObjDiv 
                                                key={idx}
                                                >
					...
					{props.data.map((row,idx) => (
						...
						<TableBodyRow 
							...
							{Object.values(row).map((value,val_idx)=>{
                                if(typeof value.indexOf == "function" && value.indexOf("dropdown") != -1){
                                    let _list = value.indexOf("dropdown0") != -1 ? props.stopTypeList : props.stopReasonList;
                                    return (
                                        <TableBodyCell
                                            key={"tableCell" + val_idx} 
                                            align="center"
                                            style={{padding : 0, backgroundColor : "#EEF9FF"}}
                                            >
                                            <EquipNOPStopSelect
                                                value={value.slice(0, value.indexOf("dropdown"))}
                                                onChange={(e)=>handleSelectVal(value.indexOf("dropdown0") != -1 ? "type" : "reason", e, idx)}
                                                disableUnderline
                                                >
                                                {_list && _list.map((raw : string, list_idx:number)=>{
                                                    return (
                                                        <MenuItem key={list_idx} value={list_idx}>{raw}</MenuItem>
                                                    );
                                                })}
                                            </EquipNOPStopSelect>
                                        </TableBodyCell>
                                ...
                                if(props.headObjList && props.headObjList.indexOf(Object.keys(row)[val_idx]) != -1){
                                    return(
                                        <TableBodyCell
                                            key={"tableCell" + val_idx}
                                            align="center"
                                            ...
                                            {Object.values(value).map((raw : any, val_obj_idx : any)=>{
                                                return (
                                                    <TableBodyObjDiv key={val_obj_idx}>
                                                        {raw}
                                                    </TableBodyObjDiv>
                                                )
                                            })}
                        ...
                        <TableBodyRow>
                            <TableBodyCell align="center">{Object.values(row)[0]}</TableBodyCell>
                            <TableBodyCell align="center">메모</TableBodyCell>
                            <TableBodyCell colSpan={Object.values(row).length - 2}>
                                <TableMemo 
...
const TableBodyRow = styled(TableRow)`
	background-color : ${(props : styledProps) => props.$selected ? `#DAEDFF` : `transparent`};
`;
...
const TableBodyCell = styled(TableCell)`
	...
	width : fit-content;
	background-color : ${(props : styledProps) => props.$cellSelected ? `#DAEDFF` : `transparent`};
	padding : 10px;
`;
